...
- 依赖前置：任何子路径修改前先做存在性检查；对象/索引不存在时，必须先创建再修改，命令顺序固定为“创建→修改”。
...
## 4.1 剧情前变量可行性审查流程（强制）
- 固定流程（仅在 thinking 内）：
  - 从用户意图生成候选动作（可执行的最小动作集合）。
  - 由候选动作推导候选变量变更（禁止直接先写剧情成功结果）。
...
- 失败改道（硬约束）：
  - 任一命令三检失败，必须改写为可执行剧情分支（降级行动/延后行动/替代行动/失败态/等待态），并重生候选动作与命令。
